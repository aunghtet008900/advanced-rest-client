...
//        console.log(response);
        ...
        function getContentType() {
            if (ct)
                return ct;
            if (!$scope.data || $scope.data.response.headers.length === 0)
                return null;
...
ArcControllers.controller('HistoryController', ['$scope', 'history', function($scope, history) {
        ...
        /**
         * It holds an lately deleted object so the user may restore it
         * until he delete another one.
         */
        $scope.latestDeleted = null;
        $scope.latestDeletedIndex = null;
        ...
        $scope.restore = function(entry, harIndex) {
            console.log(entry, harIndex);
        };
        
        $scope.deleteEntry = function(entry, harIndex) {

            var _removeEntry = function() {
                if (harIndex || harIndex === 0) {
                    $scope.latestDeletedIndex = harIndex;
                    entry.har.log.entries.splice(harIndex,1);
                    entry.har.log.pages.splice(harIndex,1);
                    //TODO: save har
                } else {
                    $scope.latestDeletedIndex = null;
                    $scope.entries.all = $scope.entries.all.filter(function(it) {
                        return it.key !== entry.key;
                    });
                    //TODO: delete from db, remove file.
                }
                $scope.latestDeleted = entry;
            };
            if (!entry.har) {
                history.get(entry.file.name).then(function(har) {
                    if (!har)
                        return;
                    try {
                        entry.har = JSON.parse(har);
                    } catch (e) {
                        return;
                    }
                }).then(_removeEntry);
...
ArcControllers.controller('SettingsController', ['$scope', 'analytics', '$timeout', function($scope, analytics, $timeout) {
        ...
        $scope.analyticsEnabledText = function() {
            return $scope.settings.analyticsEnabled === true ? 'Reporting is enabled' : 'Reporting is disabled';
        };
        $scope.syncEnabledText = function() {
            return $scope.settings.syncEnabled === true ? 'Sync is enabled' : 'Sync is disabled';
        };
    }]);
