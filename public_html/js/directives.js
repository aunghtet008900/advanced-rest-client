angular.module('arc.directives', [])
        ...
        .directive('historyItem', ['history', function(history) {
                return {
                    link: function(scope, element, attrs) {
                        var open = element[0].querySelector('[open]');
                        if (!open)
                            return;

                        if (!scope.entry.file || !scope.entry.file.name) {
                            return;
                        }

                        var fillPanel = function(panel) {
                            if (panel.classList.contains('filled')) {
                                return;
                            }
                            history.get(scope.entry.file.name).then(function(har) {
                        ...
                        var onClick = function(e) {
                            var panel = element[0].querySelector('[details]');
                        ...
                        element.on('$destroy', function() {
                            open.removeEventListener('click', onClick, false);
                        });
                    }
                };
            }])
        .directive('httpHeaderEditor', ['$q', 'Definitions', function($q, Definitions) {
                return {
                    restrict: 'E',
                    scope: {
                        'header': '=',
        ...
        .directive('hidableEditorForm', [function() {
        ...
        .directive('responseStatus', [function() {
                return {
                    restrict: 'E',
                    scope: {
                        'status': '=',
        ...
        .directive('responseRedirect', [function() {
                return {
                    restrict: 'E',
                    scope: {
                        'redirect': '=',
        ...
        .directive('responseHeaders', ['Definitions', function(Definitions) {
                return {
                    restrict: 'E',
                    scope: {
                        'headers': '='
        ...
        .directive("userProfile", ['$User', 'analytics', '$modal', function($User, analytics, $modal) {
                return {
                    restrict: 'A',
                    scope: {},
                    controller: function($scope) {
        ...
        .directive('inputSuggestions', [function() {
                return {
                    restrict: 'A',
                    scope: {
                        /**
