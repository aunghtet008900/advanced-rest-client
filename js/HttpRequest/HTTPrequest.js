 ...
 * Or:
 * <code>
 * new HttpRequest({url:'http://www.google.com', 'loadend': function(e){ ... }})
 * .execute();
 * </code>
 * 
...
BaseHttpRequest.prototype = {
    ...
    _setURLdata: function() {
        ...
        this.request_path = this.uri.uriParts.relative;
...
function ChromeHttpRequestImplementation(opts) {
    ...
    this.responseSuspectedLength = -1;
    this.responseRead = 0;
    this.payload = [];
    /**
    ...
    _readSocketData: function(readInfo) {
        ...
        if (readInfo)
            if (readInfo && readInfo.resultCode <= 0) {
                this.log(performance.now(), 'Disconnected or end of message.');
...
function HttpRequest(opts) {
    ...
    if (opts && opts.error) {
        this.addEventListener('load', opts.error);
        delete opts.error;
    }
